...
#include <amazon_msgs/RecognizeObjectFromDataAction.h>
...
class RecognizeObjectFromDataAction
{
    protected:
        ...
        actionlib::SimpleActionServer<amazon_msgs::RecognizeObjectFromDataAction> as_; 
    ...
    public:
        ...
        RecognizeObjectFromDataAction(): as_(nh_, name, boost::bind(&RecognizeObjectFromDataAction::executeCB, this, _1), false), action_name_(name)
    {
        as_.start();
    }

        ~RecognizeObjectFromDataAction(void)
        {
        }
        ...
        {
            bool success = true;
            ROS_INFO_STREAM("Received object recognition goal");
            return 
...
int main (int argc, char** argv)
{
    ...
    RecognizeObjectFromDataAction recognize_objec_from_data(ros::this_node::getName(), boost::make_shared<vector<string> >(bin_label), boost::make_shared<vector<double> >(bin_size), boost::make_shared<vector<double> >(bin_pose));
